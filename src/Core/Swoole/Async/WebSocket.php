...
class WebSocket extends Core\Swoole\Client\WebSocket
{
     ...
     * @param $event
     * @param $callable
     * @throws Core\Swoole\Http\WebSocketException
     */
    public function on($event, $callable)
    {
        switch($event)
        {
            ...
            case 'error':
                break;
            default:
                throw new Core\Swoole\Http\WebSocketException("unknow event type.");
        }
    ...
    final public function onReceive(\swoole_client $socket, $data)
    {
        ...
        if ($this->handshake)
        {
            ...
            catch (Core\Swoole\Http\WebSocketException $e)
            {
                if ($e->getCode() == Core\Swoole\Http\WebSocketParser::ERR_TOO_LONG)
                {
                    $this->socket->close();
