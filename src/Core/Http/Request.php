...
class Request extends ServerRequest
{
    ...
    {
        file_put_contents(__DIR__.'/data.txt',$request->rawContent());
    ...
    function getRequestParam($keyOrKeys = null, $default = null){
        if($keyOrKeys !== null){
            ...
            return array_merge($this->getParsedBody(),$this->getQueryParams(),$this->getBodyParams());
        }
    }

    public function getBodyParams(){
        if($this->getSwooleRequest()->rawContent()) {
            $params = json_decode($this->getSwooleRequest()->rawContent(),true);
            if(json_last_error())  {
                return [];
            } else {
                return $params;
